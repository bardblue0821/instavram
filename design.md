...
### 新規登録時にメールアドレス認証を行う（ここ）
- 目的: メール+パスワード仮登録後に本人性を確認して本登録を完了させる。
- 方針: まずは Firebase Auth 標準の確認リンク方式で実装（最短・無料）。
- 制約:
  - 本登録が済んでいないユーザーは、ログイン画面にしかアクセスができない
  - 新規登録ボタンを押すといかが起きるようにする
    - 仮登録用のメールが飛ぶ
    - 以下の表示
      - 未だ仮登録であること
      - リンクをクリックして本登録してほしいこと
      - 再送リンク
    - この表示は画面が切り替わるまで表示する。
    - 仮登録状態ではログインはできていない。したがってタイムラインなど、ログイン画面以外の画面は依然としてみることができない。

### 登録時のフロー
- 未登録状態
  - ログイン画面にしかアクセスできない
  - アドレスとパスワードを設定することで、二段階認証の連絡が飛ぶ
  - 新規登録をしたい人は、メールアドレスの入力だけが必要。パスワードの欄はない。メールアドレスの認証後に入力するから。
- 仮登録状態
  - ログイン画面にしかアクセスができない
